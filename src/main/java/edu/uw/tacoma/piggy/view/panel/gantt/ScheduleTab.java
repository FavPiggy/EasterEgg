...
public class ScheduleTab extends javax.swing.JPanel {
	...
	private TaskListData taskData = null;
	...
	public ScheduleTab(TaskListData theTaskData) {
		this.taskData = theTaskData;
		this.taskData.addSelectionChangeListener(new SelectionChangeListener() {
		...
		pnlTreeView.add(taskTree);
			int index = 0;
			if (theTaskData.getTimeUnit() == TimeUnit.Hour) {
				index = 0;
			} else if (theTaskData.getTimeUnit() == TimeUnit.AllDay) {
				index = 1;
			} else if (theTaskData.getTimeUnit() == TimeUnit.Day) {
				index = 2;
			} else if (theTaskData.getTimeUnit() == TimeUnit.Week) {
				index = 3;
			} else if (theTaskData.getTimeUnit() == TimeUnit.Month) {
				index = 4;
			} else if (theTaskData.getTimeUnit() == TimeUnit.Year) {
				index = 5;
			}
	...
	private void cmbTimeUnitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmbTimeUnitActionPerformed
		if (cmbTimeUnit.getSelectedItem().equals("Hour")) {
			//taskData.initHourlyModel();
		} else if (cmbTimeUnit.getSelectedItem().equals("AllDay")) {
			//taskData.initAllDayModel();
		} else if (cmbTimeUnit.getSelectedItem().equals("Day")) {
			//taskData.initDailyModel();
		} else if (cmbTimeUnit.getSelectedItem().equals("Week")) {
			//taskData.initWeeklyModel();
		} else if (cmbTimeUnit.getSelectedItem().equals("Month")) {
		//	taskData.initMonthlyModel();
		} else if (cmbTimeUnit.getSelectedItem().equals("Year")) {
		//	taskData.initYearModel();
		}
		taskTree.setTreeModel(taskData.getGanttModel().getTaskTreeModel());
		taskTree.updateUI();
		taskTree.expandAllTreeNode((DefaultMutableTreeNode) taskData.getGanttModel().getTaskTreeModel().getRoot());
